"""
file:       Calculate_relative_thickness_(log-ratio).py
author:     Michael Entrup b. Epping (michael.entrup@wwu.de)
version:    20170306
info:       A script that calculates the thickness map using two images.
            The second image (full signal) is devided by the first one (ZLP-filtered).
...
def get_setup():
    ...
    dialog = GenericDialog('Relative thickness setup')
    ...
    dialog.addMessage('Full signal is divided by ZLP-filtered.')
    dialog.addChoice('ZLP-filtered', image_titles, image_titles[0])
    dialog.addChoice('Full signal', image_titles, image_titles[1])
    dialog.showDialog()
...
def run_script():
    ...
    img_ratio.setTitle('Relative thickness map [ln(%s divided by %s)]' % (img2.getShortTitle(),
                                                                          img1.getShortTitle()
                                                                         )
                      )
