...
class ThreeWindow:
    ...
    The background signal at the post-edge image is estimated by fitting a power law function to the pre-edge images.
    The linear representation of the power law is used. The slope and y-intercept of the linear function are calculated using the two-point-form.
    ...
    def __init__(self, pre1, pre2, post):
        ...
        print 'Energy-losses extracted form image titles: %deV, %deV and %deV' % (self.eloss_pre1, self.eloss_pre2, self.eloss_post)
    ...
    def calc_map(self):
        ...
        E2 = self.eloss_pre2
        Es = self.eloss_post
        self.lnAs = [self.calc_lnA(Nb1, Nb2, E1, E2) for Nb1, Nb2 in zip(self.pre1_imp.getProcessor().getPixels(), self.pre2_imp.getProcessor().getPixels())]
        self.rs = [self.calc_r(Nb1, Nb2, E1, E2) for Nb1, Nb2 in zip(self.pre1_imp.getProcessor().getPixels(), self.pre2_imp.getProcessor().getPixels())]
        self.sigs = [self.calc_sig(sig, lnA, r, Es) for sig, lnA, r in zip(self.post_imp.getProcessor().getPixels(), self.lnAs, self.rs)]

    def calc_sig(self, sig, lnA, r, Es):
        '''A helper function used by calc_map().'''
        if not Float.isNaN(lnA) and not Float.isNaN(r) and sig > 0:
            return float(sig) - calc_pow(Es, lnA, r)
        else:
            return Float.NaN
    ...
    def calc_snr(self):
        ...
        if self.sigs and self.rs and self.lnAs:
            self.calc_vars()
            self.calc_hs()
            vars_sig = [self._calc_var_sig(sig, h, lnA, r) for sig, h, r, lnA in zip(self.sigs, self.hs, self.rs, self.lnAs)]
            self.snrs = [self._calc_snr(sig, var_sig) for sig, var_sig in zip(self.sigs, vars_sig)]

    def calc_vars(self):
        '''A helper function used by calc_snr().'''
        self.variances = [self._calc_var(lnA, r) for lnA, r in zip(self.lnAs, self.rs)]
    ...
    def calc_hs(self):
        '''A helper function used by calc_snr().'''
        self.hs = [self._calc_h(r, lnA, var) for r, lnA, var in zip(self.rs, self.lnAs, self.variances)]
    ...
    def get_result(self):
        '''Returns the ImagePlus objects that represent the elemental map, the SNR and the maps of the parameters r and ln(a).
        ...
        if not self.sigs:
            self.calc_map()
        lnA_imp = ImagePlus('Map of parameter ln(a)', FloatProcessor(self.post_imp.getWidth(), self.post_imp.getHeight(), array(self.lnAs, 'f')))
        r_imp = ImagePlus('Map of parameter r', FloatProcessor(self.post_imp.getWidth(), self.post_imp.getHeight(), array(self.rs, 'f')))
        sig_imp = ImagePlus('Elemental map %deV - %s' % (self.eloss_post, self.post_imp.getShortTitle()), FloatProcessor(self.post_imp.getWidth(), self.post_imp.getHeight(), array(self.sigs, 'f')))
        if not self.snrs:
            self.calc_snr()
        snr_imp = ImagePlus('SNR %deV - %s' % (self.eloss_post, self.post_imp.getShortTitle()), FloatProcessor(self.post_imp.getWidth(), self.post_imp.getHeight(), array(self.snrs, 'f')))
        IJ.showProgress(1.0)
